...
// check if str1 is greate than the str2 
	...
	function Signatories($data)
	{
		...
		$this->MultiCell(100, 5,$data['client_name'], $this->border, 'L', 0, 0, '', '', true);
		...
		$this->MultiCell(100, 4,'SECOND PARTY', $this->border, 'L', 0, 0, '', '', true);

		//First Party
		$this->MultiCell(90, 4,'FIRST PARTY', $this->border, 'L', 0, 1, '', '', true);
		
		// BY:
		$this->MultiCell(100, 4,'By:', $this->border, 'L', 0, 0, '', '', true);
		...
		//Client Sign By
		$this->MultiCell(100, 3,strtoupper($data['client_sign_by']), $this->border, 'L', 0, 0, '', '', true);
		$this->SetFont('helvetica','',10);
		...
		if($data['client_sign_by'] == '' || $data['client_sign_by'] == null)
		{
			...
			$this->MultiCell(70, 3,'Authorized Signature Over Printed Name', $this->border, 'L', 0, 0, '', '', true);
		}
		else
		{
			...
			$this->MultiCell(100, 3,$data['client_sign_position'],$this->border, 'L', 0, 0, '', '', true);
		}
...
/*ob_start();*/
	...
	$pdf->MultiCell(40, 5, date('F d, Y',strtotime($contract['doc_date'])), $border, 'L', 0, 1, '', '', true);
	...
	if ($content !== '') 
	{
		...
		$html = '<ul>';
		if ($has_preferred == 1) $html .= '<li>JLR Construction and Aggregates Inc. is the first preferred concrete supplier.</li>';
		$html .= '<li>'.$content.'</li>';
		$html .= '</ul>';
		$pdf->writeHTML($html,true,false,true,false);	
		$pdf->Ln(2);
	...
	**/
	if (count($pump_charges) > 0) 
	{
		...
		*/
			...
			foreach ($pump_charges as $pc) 
			{
				if ($p->charge_id === $pc->charge_id) 
				{
					...
					foreach ($arr_pump as $pp) 
					{
						...
						$td_pump .= '<td style=" border-bottom: 0.5 dashed #000; border-right:1px solid #000;">'.$pump_value.'</td>';
					}

					$tbl_details .= '<tr><td style=" border-bottom: 0.5 dashed #000; border-right:1px solid #000;">'.$td_charge.'</td>'.$td_pump.'</tr>';
					...
					if (strpos($p->charge_desc,'Floor Level Surcharge') !== FALSE) 
					{
						...
						{ 
							...
							foreach ($arr_pump as $p) 
							{
								...
								$pump_value .='<td style=" border-bottom: 0.5 dashed #000; border-right:1px solid #000;">'.($amt != ''?peso($amt):'').'</td>';
							}

							$tbl_details .= '<tr><td align="center" style=" border-bottom: 0.5 dashed #000; border-right:1px solid #000;">'.$desc.'</td>'.$pump_value.'</tr>';
						}
					}
		...
		#PUMP RATES DEFAULT
		$tbl = '<table border="1" cellpadding="3" align="left">
			<tbody>
				<tr>
					<td style=" border-bottom: 0.5 dashed #000; border-right:1px solid #000;">Pouring Rate, minimum</td>
					<td style=" border-bottom: 0.5 dashed #000; border-right:1px solid #000;">20-25 ,m³/hour</td>
					<td style=" border-bottom: 0.5 dashed #000; border-right:1px solid #000;">10 m³/hour</td>
				</tr>
				<tr>
					<td style=" border-bottom: 0.5 dashed #000; border-right:1px solid #000;">Waiting Time, maximum</td>
					<td style=" border-bottom: 0.5 dashed #000; border-right:1px solid #000;">2 hours</td>
					<td style=" border-bottom: 0.5 dashed #000; border-right:1px solid #000;">2 hours</td>
				</tr>
				<tr>
					<td style=" border-bottom: 0.5 dashed #000; border-right:1px solid #000;">Waiting Time<br> Charges in excess of 2hrs</td>
					<td style=" border-bottom: 0.5 dashed #000; border-right:1px solid #000;">2,500.00 /hour</td>
					<td style=" border-bottom: 0.5 dashed #000; border-right:1px solid #000;">2,500.00 /hour</td>
				</tr>
			</tbody>
		</table>';
